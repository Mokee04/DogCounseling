...
drive_folder_id = st.secrets['drive_folder_id']  # Google Drive 폴더 ID 가져오기
...
def save_chat_data(user_id):
    ...
    try:
        ...
        print(f"채팅 데이터 Google Drive에 저장 완료: {filename} (ID: {file_id})")
        return True
    except Exception as e:
    ...
    try:
        ...
        if model_file_id:
            ...
            if counselor is None:
                ...
                st.session_state['counselor'].define_model(model_name="models/gemini-2.0-flash-thinking-exp-01-21")
            ...
            # 초기 메시지 추가 (고유 ID 부여)
            initial_message = st.session_state['counselor'].send_question(
                "이제 반려견 양육 상담을 시작해 주세요. 먼저 '반려견 양육 상담 사전조사 내용'을 기반으로 프로파일링을 시도하고, 그 내용과 함께 보호자에게 어떻게 상담을 할 것인지 소개를 해주세요."
            )
            message_id = str(uuid.uuid4())
            st.session_state['chat_history'] = [{
                "id": message_id,
        ...
        st.session_state['user_info'] = process.resp_dict['pet_name']  # 반려견 이름 저장
    ...
    if 'counselor' in st.session_state and st.session_state['user_id']:
        ...
        if st.session_state.get('model_file_id'):
            ...
            model_saved = st.session_state['counselor'].save_chatmodel(
                ...
                file_id=st.session_state['model_file_id'],  # 기존 ID 전달
                credentials=credentials
            )
        else:
            # 새 파일 생성
...
if not st.session_state['is_logged_in']:
    ...
    with st.sidebar:
        ...
        if st.button("상담 내용 저장"):
            ...
            if st.session_state['user_id']:
                ...
                if model_saved and chat_data_saved:
                    st.success("상담 내용과 피드백이 Google Drive에 저장되었습니다.")
                elif model_saved:
                    st.warning("상담 내용은 저장되었으나 채팅 데이터 저장에 실패했습니다.")
                else:
                    st.error("저장 중 오류가 발생했습니다.")
    ...
    def render_message(message):
        """
        채팅 메시지와 피드백 버튼을 동시에 그려주는 함수
        
        메시지와 버튼이 항상 함께 렌더링되도록 보장하여 UI 안정성 향상
        스트림릿에서 UI 요소가 분리되면 비동기로 로드되어 버튼이 늦게 뜨는 문제 해결
        """
        if message["role"] == "assistant":
            # ID 확인/생성 - 없으면 새로 만들어서 피드백 연결 보장
            if "id" not in message or message.get("id") is None:
            ...
            # 버튼 강조 상태 설정 - 이미 클릭된 버튼은 파란색으로 표시
            ...
            # 한 컨테이너에 메시지와 버튼 함께 배치 - 동시 로딩 보장
            with st.container():
                # 11:0.5:0.5 비율로 열 설정 - 버튼 간격 좁게 설정
                cols = st.columns([11, 0.5, 0.5])
                ...
                # 좋아요 버튼 - 메시지 ID로 고유키 생성하여 중복 방지
                with cols[1]:
                    st.button("👍", key=f"like_{message_id}", on_click=handle_like, args=(message_id,), type=like_style)
                
                # 싫어요 버튼 - 메시지 ID로 고유키 생성하여 중복 방지
                with cols[2]:
                    st.button("👎", key=f"dislike_{message_id}", on_click=handle_dislike, args=(message_id,), type=dislike_style)
        else:
            # 사용자 메시지는 단순 표시 (피드백 불필요)
            with st.chat_message(message["role"]):
                st.write(message["content"])
